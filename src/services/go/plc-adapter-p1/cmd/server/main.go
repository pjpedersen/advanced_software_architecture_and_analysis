...
import (
	...
	"github.com/i40/production-system/services/plc-adapter-p1/internal/api"
	"github.com/i40/production-system/services/plc-adapter-p1/internal/config"
	"github.com/i40/production-system/services/plc-adapter-p1/internal/service"
)

func main() {
	log.Println("Starting PLC Adapter for Injection Molding Cell (P1)...")
	...
	consumer, err := sharedkafka.NewConsumer(sharedkafka.ConsumerConfig{
		BootstrapServers: cfg.KafkaBootstrapServers,
		GroupID:          "plc-adapter-p1-consumer",
		Topics:           []string{"production.plan"},
